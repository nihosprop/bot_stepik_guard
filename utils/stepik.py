...
class AsyncStepikCommenter:
    ...
    async def get_units(self, section_id: int) -> List[int]:
        section_url = f'https://stepik.org/api/sections/{section_id}'
        async with self.session.get(
            url=section_url,
            headers={'Authorization': f'Bearer {self.token}'}) as resp:
            # logger.info((await resp.json())['sections'])
            units = (await resp.json())['sections'][0]['units']
    ...
    async def get_steps(self, unit_id: int) -> List[int]:
        ...
        async with self.session.get(url=lesson_url) as resp:
            steps = (await resp.json())['lessons'][0]['steps']
            logger_stepik.info(f'{steps=}')
            return steps
