...
async def cmd_start(msg: Message,
    ...
    text = (f'<b>Приветствую, {tg_nickname}!</b>\n'
            ...
            f'1. Отключить не нужные уведомления можно в настройках.\n'
            f'2. Удаление токсичных комментариев можно включить в '
            f'настройках.\n'
            f'3. Посмотреть текущие настройки можно в боковом меню.\n\n'
            f'Важно❗\n'
    ...
    if await redis_service.check_user(tg_user_id=user_tg_id):
        ...
        await redis_service.add_user(tg_user_id=user_tg_id)
        logger.info(f'New user {tg_nickname} added.')
